...
const DocumentManager: React.FC = () => {
  ...
  const getFileIcon = (type: string) => {
    switch (type.toLowerCase()) {
      case "pdf":
        return <FileText className="h-6 w-6 text-brand-ink" />;
      case "docx":
      case "doc":
        return <FileText className="h-6 w-6 text-brand-ink" />;
      case "jpg":
      case "jpeg":
      case "png":
        return <Image className="h-6 w-6 text-brand-neon" />;
      default:
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-brand-ink mb-2">Document Manager</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 2xl:grid-cols-6 4xl:grid-cols-8 gap-6 mb-8">
        ...
        <Card className="text-center" padding="lg">
          <div className="text-2xl font-bold text-brand-neon mb-1">24.8 MB</div>
          <div className="text-sm text-brand-slate">Storage Used</div>
        </Card>
        <Card className="text-center" padding="lg">
          <div className="text-2xl font-bold text-brand-ink mb-1">3</div>
          <div className="text-sm text-brand-slate">Starred Items</div>
        </Card>
        <Card className="text-center" padding="lg">
          <div className="text-2xl font-bold text-brand-ink mb-1">5</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1">
          <Card>
            <h3 className="text-lg font-semibold text-brand-ink mb-4">Categories</h3>
            <div className="space-y-2">
              {categories.map((category) => (
                <button
                  key={category.id}
                  onClick={() => setSelectedCategory(category.id)}
                  className={`w-full flex items-center justify-between p-3 rounded-[1.5rem] text-left transition-colors ${
                    selectedCategory === category.id
                      ? "bg-primary-50 text-primary-700 border border-primary-200"
                      : "hover:bg-brand-mist text-brand-ink"
                  }`}
                >
                  <div className="flex items-center space-x-3">
                  ...
                  <span className="text-xs bg-brand-mist text-brand-slate px-2 py-1 rounded-[2rem]">
                    {category.count}
                  </span>
                </button>
              ))}
            ...
            {/* Quick Actions */}
            <div className="mt-8">
              <h4 className="text-sm font-semibold text-brand-ink mb-3">Quick Actions</h4>
              <div className="space-y-2">
        ...
        <div className="lg:col-span-3">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex border border-brand-ink/20 rounded-[1.5rem]">
                <button
                ...
                >
                  <div className="w-4 h-4 grid grid-cols-2 gap-0.5">
                    <div className="bg-current rounded-[1rem]"></div>
                    <div className="bg-current rounded-[1rem]"></div>
                    <div className="bg-current rounded-[1rem]"></div>
                    <div className="bg-current rounded-[1rem]"></div>
                  </div>
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      <button className="opacity-0 group-hover:opacity-100 transition-opacity">
                        <Trash2 className="h-4 w-4 text-gray-400 hover:text-brand-ink" />
                      </button>
                    </div>
                  </div>

                  <h3 className="font-semibold text-brand-ink mb-2 truncate">{doc.name}</h3>
            ...
            <Card>
              <div className="space-y-4">
                {filteredDocuments.map((doc) => (
                  <div
                    key={doc.id}
                    className="flex items-center justify-between p-4 hover:bg-brand-mist rounded-[1.5rem] transition-colors group"
                  >
                    <div className="flex items-center space-x-4">
                      {getFileIcon(doc.type)}
                      <div>
                        <h3 className="font-medium text-brand-ink">{doc.name}</h3>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Button variant="ghost" size="sm" className="opacity-0 group-hover:opacity-100">
                        <Trash2 className="h-4 w-4 text-brand-ink" />
                      </Button>
                    </div>
                  </div>
                ))}
          ...
          {filteredDocuments.length === 0 && (
            <Card className="text-center" padding="lg">
              ...
              <h3 className="text-lg font-medium text-brand-ink mb-2">No documents found</h3>
